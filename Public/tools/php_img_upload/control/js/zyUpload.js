...
(function($,undefined){
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var _Dir = "/Public/tools/php_img_upload/";
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="'+_Dir+'control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
...
//				}else{
					for (var i = 0, file; file = files[i]; i++) {
						...
						}else if(file.type != "image/jpeg" && file.type != "image/png" && file.type != "image/gif" && file.type != "image/bmp") {
							alert('您这个"'+ file.name +'"格式的图片不支持');		
						}else{
							// 在这里需要判断当前所有文件中
							arrFiles.push(file);	
						}
...
//				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if(para.edit){  // 显示编辑按钮
					editHtml = '<span class="file_edit" data-index="'+file.index+'" title="预览"></span>';
				}
				if(para.del){  // 显示删除按钮
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = _Dir + "control/images/fileType/";
			...
			this.createPopupPlug = function(imgSrc, index){
				...
				$("body").zyPopup({
					...
					onTailor   :   function(val){     // 回调返回裁剪的坐标和宽高的值
						...
						$.getScript( _Dir + "jquery.json-2.4.js", function(){
			...
			this.createCorePlug = function(){
				var params = {
							...
							if($(".file_edit").length>0){
								if($("head").html().indexOf("zyPopup")<0){  // 代表没有加载过js和css文件
									...
									$("<link>").attr({ rel: "stylesheet",
								        type: "text/css",
								        href: _Dir + "zyPopup/css/zyPopup.css"
								    }).appendTo("head");
									$.getScript(_Dir + "zyPopup/js/zyPopup.js", function(){
										// 编辑方法
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>上传成功，文件地址是：" + json.data.img_src + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
